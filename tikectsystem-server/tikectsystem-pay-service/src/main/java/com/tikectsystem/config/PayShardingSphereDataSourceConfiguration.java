...
/**
 * 支付服务 ShardingSphere 数据源配置。
 * <p>
 * 先解析 ShardingSphere YAML 中的 Spring 占位符，再创建数据源，避免生产库密码写死在配置文件中。
 */
@Configuration
public class PayShardingSphereDataSourceConfiguration {

    private static final String DEFAULT_PROFILE = "local";

    private static final String REQUIRED_PLACEHOLDER_PREFIX = "${PAY_";

    private final Environment environment;

    private final ResourceLoader resourceLoader;

    public PayShardingSphereDataSourceConfiguration(final Environment environment, final ResourceLoader resourceLoader) {
    ...
    public DataSource dataSource() throws SQLException, IOException {
        ...
        String defaultLocation = "classpath:shardingsphere-pay-" + activeProfile + ".yaml";
        String location = environment.resolvePlaceholders(environment.getProperty("pay.shardingsphere.config-location", defaultLocation));
    ...
    private void validateRequiredPlaceholdersResolved(String resolvedConfigContent) {
        if (resolvedConfigContent.contains(REQUIRED_PLACEHOLDER_PREFIX)) {
            throw new IllegalStateException("Pay ShardingSphere datasource environment variables are not fully configured");
        }
    }
}
